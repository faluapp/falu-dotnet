...
public class MessageTemplateModelJsonConverterTests
{
    ...
    {
        var request = JsonSerializer.Deserialize(expected, FaluJsonSerializerContext.Default.MessageTemplateValidationRequest);
        var actual = JsonSerializer.Serialize(request, FaluJsonSerializerContext.Default.MessageTemplateValidationRequest);
